...
DataSet3 = spark.createDataFrame(
    [
        Row(location="Oslo", people=[Row(firstName="Harald", lastName="Haraldsen"), Row(firstName="Mark", lastName="Hunter")]),
...
DataSet4 = spark.createDataFrame(
    [
        Row(location="Copenhagen", people=[Row(firstName="Roger", nobleName="of house Henriksen"), Row(firstName="Hans", nobleName="of house Dale")]),
    ]
)
...
from pyspark.sql.functions import concat, col, DataFrame, lit, explode

def explodePeople(df: DataFrame) -> DataFrame:
    """Explode people column."""
    return df.withColumn("ex_people", explode("people")).select("location", "ex_people.*")

def addInitials(df: DataFrame) -> DataFrame:
    """Add initials column."""
    df_new = df.withColumn("intials", concat(col("firstName").substr(0,1), col("lastName").substr(0,1)))
    return df_new
...
def nameTransformation(data: DataFrame, correctNameColumns: callable = lambda df: df) -> DataFrame:
